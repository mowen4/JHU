...
class Node {
   public:
    ...
    virtual std::shared_ptr<Node> derivative(
...
class Constant : public Node {
   public:
    ...
    std::shared_ptr<Node> derivative(
...
/// @param os
void Constant::print(std::ostream& os) const { os << value; }
...
/// @param varName
Variable::Variable(const std::string& varName) { name = varName; }

/// @brief Variable evaluate implementation returns the double stored in the
/// <string, double> map for the given name.
/// @return
double Variable::evaluate() const { return variables[name]; }
...
/// @return
double Add::evaluate() const { return left->evaluate() + right->evaluate(); }
...
std::shared_ptr<Node> Add::derivative(const std::string& variable) const {
    return std::make_shared<Add>(left->derivative(variable),
                                 right->derivative(variable));
}

/// @brief Add print function implementation. Prints the left and right node
/// with a + in between. Whole expression is wrapped in parentheses.
/// @param os
...
/// @brief Constructor for Sub node. Sets the values for the left and right
/// nodes.
/// @param leftNode
/// @param rightNode
...
/// @return
double Sub::evaluate() const { return left->evaluate() - right->evaluate(); }
...
std::shared_ptr<Node> Sub::derivative(const std::string& variable) const {
    return std::make_shared<Sub>(left->derivative(variable),
                                 right->derivative(variable));
}

/// @brief Sub print function implementation. Prints the left and right node
/// with a - in between. Whole expression is wrapped in parentheses.
...
/// the left node to the recursive evlauation of the right node.
double Mul::evaluate() const { return left->evaluate() * right->evaluate(); }
...
/// that is the same as the current left node and the right node is the
/// recursive derivative of current right node with respect to the given
/// variable. The right node of the returned std::shared_ptr<Add> is also a new
/// std::shared_ptr<Mul>. This one has a left node that is the recursive
/// derivative of the current left node and the right node is the same as the
/// current right node.
...
/// @brief Mul print function implementation. Prints the left and right node
/// with a * in between. Whole expression is wrapped in parentheses.
...
/// the left node from the recursive evlauation of the right node.
double Div::evaluate() const { return left->evaluate() / right->evaluate(); }

/// @brief Derivative of the Div node returns a std::shared_ptr<Div>. The left
/// node of this returned value is a std::shared_ptr<Sub> and the right node is
/// a std::shared_ptr<Mul>. In the <Sub> node, the left node is a
/// std::shared_ptr<Mul> where the left node is a recursive derivative of the
/// current left node with respect to the given variable and the right node is
/// the same as the current right node. The right node in the <Sub> node is a
/// second std:shared_ptr<Mul> which holds the current left node as its left
/// value and the recursive derivative of the current right node with respect to
/// the given variable as its right node. The right node of returned <Div> node
/// is a std::shared_ptr<Mul> whose left and right nodes are the current right
/// node.
/// @param variable
/// @return
...
/// @brief Div print function implementation. Prints the left and right node
/// with a / in between. Whole expression is wrapped in parentheses.
...
int main() {
    std::shared_ptr<Node> node = std::make_shared<Add>(
        std::make_shared<Mul>(std::make_shared<Constant>(2.3),
                              std::make_shared<Variable>("Xray")),
        std::make_shared<Mul>(
            std::make_shared<Variable>("Yellow"),
            std::make_shared<Sub>(std::make_shared<Variable>("Zebra"),
                                  std::make_shared<Variable>("Xray"))));
    ...
    std::cout << "Derivative of the tree with respect to the variable Xray: "
              << std::endl;
