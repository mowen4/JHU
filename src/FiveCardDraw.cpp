...
struct Card {
    ...
    // override < > operators to allow for sort function calls to know how to
    // sort
...
void Hand::setRank() {
    ...
    if (allCardsUnique) {
        ...
        if ((cards[4].rank - cards[0].rank) == 4 ||
            (cards[4].rank == 14 && cards[0].rank == 2 && cards[1].rank == 3 &&
...
void PokerGame::assignSeats() {
    ...
    // however many are human
    for (int i = 0; i < numberOfHumans; i++) {
        playerHands[i].setHuman();
    }
    // shuffle seats
    auto rng = std::default_random_engine{};
    std::shuffle(std::begin(playerHands), std::end(playerHands), rng);
}
...
/// @brief Sets the number of total players for the game
/// @param n number of players as an int (2-8)
void PokerGame::setNumberOfPlayers(int n) { numberOfPlayers = n; }

/// @brief Sets number of humans in the game
/// @param n number of humans as int (0-8)
void PokerGame::setNumberOfHumans(int n) { numberOfHumans = n; }
